...
def sim(z_i, z_j):
    ...
    norm_dot_product = torch.dot(z_i, z_j) / (norm_i * norm_j)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        sim_k = sim(z_k, z_k_N)
        num_k = torch.exp(sim_k / tau)

        den_k = 0
        for m in range(2 * N):
            if m != k:
                den_k += torch.exp(sim(z_k, out[m]) / tau)
        loss_k = -torch.log(num_k / den_k)

        sim_kN = sim(z_k_N, z_k)
        num_kN = torch.exp(sim_kN / tau)

        den_kN = 0
        for m in range(2 * N):
            if m != k + N:
                den_kN += torch.exp(sim(z_k_N, out[m]) / tau)
        
        loss_kN = -torch.log(num_kN / den_kN)

        total_loss += (loss_k + loss_kN)
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
...
def sim_positive_pairs(out_left, out_right):
    ...
    # HINT: torch.linalg.norm might be helpful.                                  #
    ##############################################################################
    out_left_norm = out_left / torch.linalg.norm(out_left, dim=1, keepdims=True)
    out_right_norm = out_right / torch.linalg.norm(out_right, dim=1, keepdims=True)
    
    pos_pairs = (out_left_norm * out_right_norm).sum(dim=1, keepdims=True)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return pos_pairs
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    sim_pos_all = torch.cat([sim_pos, sim_pos], dim=0)
    
    # Step 3: Compute the numerator value for all augmented samples.
    numerator = torch.exp(sim_pos_all / tau)

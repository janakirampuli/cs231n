...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    ############################################################################
    processed_imgs = [clip_preprocess(Image.fromarray(img)) for img in images]
    image_input = torch.stack(processed_imgs).to(device) # (B, ...)
    ...
    pred_classes = [class_texts[i] for i in torch.argmax(similarity, dim=0)]
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        processed_imgs = [clip_preprocess(Image.fromarray(img)) for img in images]
        image_input = torch.stack(processed_imgs).to(device)
        image_features = clip_model.encode_image(image_input)

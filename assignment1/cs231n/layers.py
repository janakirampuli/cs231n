...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    x_norm = (x - mean) / std
    out = gamma * x_norm + beta
    cache = (x, mean, var, std, x_norm, gamma, beta, eps)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return out, cache
...
def layernorm_backward(dout, cache):
    ...
    # still apply!                                                            #
    ###########################################################################
    x, mean, var, std, x_norm, gamma, beta, eps = cache
    N, D = dout.shape
    ...
    dvar = np.sum((-0.5) * (dx_norm) * (x - mean) * (ivar**3), axis=1, keepdims=True)
    
    dmean = -1.0 * (np.sum(dx_norm * (ivar), axis=1, keepdims=True) + (dvar/N) * np.sum(2 * (x - mean), axis=1, keepdims=True))

    dx = (dx_norm * ivar) + (dvar * 2.0 * (x - mean) / N) + (dmean / N)
